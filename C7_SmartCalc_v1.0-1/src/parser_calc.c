...
int pars(char *str, Stack_t **stack) {
  ...
  while (*(str + i) != '\0') {
    ...
    if (*(str + i) == '+') {
      ++i;
      if (prev == '=' || prev == '(') {
        push(&temp_stack, '&', 0.0);  //унарный плюс
        continue;
      }
      ...
      push(&temp_stack, '+', 0.0);  //сложение
      continue;
    }
    if (*(str + i) == '-') {
      ++i;
      if (prev == '=' || prev == '(') {
        push(&temp_stack, '_', 0.0);  //унарный минус
        continue;
      }
      if (!isdigit(prev) && prev != ')' && prev != 'x') {
        rc = PARSING_ERROR;
